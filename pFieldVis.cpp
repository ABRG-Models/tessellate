...
// initialise with random field
    if (Lcontinue)
	{
        ...
        for (unsigned int j=0;j<numpoints;j++) {
            ...
            FLT choice = ruf.get();
            if (choice > 0.5) {
                //S[j].NN[h.vi] = - ruf.get() * aNoiseGain * exp(-100.0 * h.r*h.r)  + nnInitialOffset;
                //S[j].CC[h.vi] = - ruf.get() * aNoiseGain * exp(-100.0 * h.r*h.r) + ccInitialOffset;
        ...
        for (unsigned int j=0;j<numpoints;j++) {
            ...
            // normal value. Close to boundary, noise is less.
                ...
                if (h.distToBoundary > -0.5) { // It's possible that distToBoundary is set to -1.0
                    ...
                    //S[j].NN[h.vi] = (S[j].NN[h.vi] - nnInitialOffset) * bSig + nnInitialOffset;
                    //S[j].CC[h.vi] = (S[j].CC[h.vi] - ccInitialOffset) * bSig + ccInitialOffset;
                    S[j].NN[h.vi] = (S[j].NN[h.vi] - nnInitialOffset) * bSig;
                    S[j].CC[h.vi] = (S[j].CC[h.vi] - ccInitialOffset) * bSig;
                } //end of if on boundary distance
            }//end of loop over region
