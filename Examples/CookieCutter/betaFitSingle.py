...
def beta_distribution_full(x, a, b):
    x_ = (x+1)*0.5
    y_ = (1./scipy.special.beta(a, b)) * np.power(x_, a-1) * np.power(1-x_, b-1)
    return y_

# Symmetric version of the above in which b=a
def beta_distribution_full_sym(x, a):
    x_ = (x+1)*0.5
    y_ = (1./scipy.special.beta(a, a)) * np.power(x_, a-1) * np.power(1-x_, a-1)
    return y_
...
# Load histogram data
hdata = np.genfromtxt(logpath + '/correlate0.data')
...
h_fit1 = beta_distribution_full_sym(x, *popt1) * binw
# Compute fit quality
normh_fit1 = np.sum(np.power(h_fit1,2))
fit_sos1 = np.sum(np.power((h_fit1 - h_prob), 2)*beta_distribution_full_sym(x,0.1))
# initial guess for the parameters
iparams2 = np.array([0.1])
popt2, pcov2 = curve_fit(beta_distribution_full_sym, x, h, iparams2,  method='lm')
# Note I transform the fitted vales from PDF to probability to be compared with h_prob:
h_fit2 = beta_distribution_full_sym(x, *popt2) * binw
# Compute fit quality
normh_fit2 = np.sum(np.power(h_fit2,2))
fit_sos2 = np.sum(np.power((h_fit2 - h_prob), 2)*beta_distribution_full_sym(x,0.1))
