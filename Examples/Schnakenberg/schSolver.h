...
// method to calculate the Laplacian
    ...
    void compute_dAdt (std::vector<FLT>& A_, std::vector<FLT>& dAdt)
    {
        ...
        for (unsigned int h=0; h<this->n; ++h) {
            dAdt[h] = this->k1 - (this->k2 * A_[h])
                + (this->k3 * A_[h] * A_[h] * this->B[h]) + this->D_A * this->lapA[h];
        }
    }
    ...
    void compute_dBdt (std::vector<FLT>& B_, std::vector<FLT>& dBdt)
    {
        ...
        for (unsigned int h=0; h<this->n; ++h) {
            // G = k4        - k3 A^2 B
            dBdt[h] = this->k4 - (this->k3 * this->A[h] * this->A[h] * B_[h]) + this->D_B * this->lapB[h];
        }
    }
  ...
  //function to timestep coupled equations option to set boundary to constant value
    ...
    void setParams(FLT D_A, FLT D_B, FLT k1, FLT k2, FLT k3, FLT k4) {
        this->D_A = D_A;
        this->D_B = D_B;
        this->k1 = k1;
        this->k2 = k2;
