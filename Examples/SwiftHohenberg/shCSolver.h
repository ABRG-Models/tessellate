...
// method to calculate the Laplacian
  ...
  // function to compute the derivative
     void compute_dpsidt(vector<complex<FLT>>& inPsi, vector<complex<FLT>>& dpsidt, vector<complex<FLT>> inPhi, FLT epsilon, FLT g) {
        ...
        for (int h=0; h < this->n; h++) {
            FLT psimod = abs(phi[h]);
            FLT psimodsq = psimod*psimod;
          dpsidt[h] = -2.0f*this->phi[h] - lapPhi[h] + (epsilon - 1.0f)*psi[h] +  (1.0f - g) * psimodsq*psi[h];
        }
    }
  ...
  //function to time step Neumann b.c.s
  ...
  //function to timestep coupled equations option to set boundary to constant value
    void step(FLT dt, FLT epsilon, FLT g, vector<complex<FLT>> oldPsi, int steps, int numAdjust)
    {
