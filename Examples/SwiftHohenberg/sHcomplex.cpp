...
// if (skipMorph) return 0;
    ...
    for (auto &h : S.Hgrid->hexen) {
        S.psi[h.vi] = polar (1.0, - ruf.get() * 2.0f * PI);
        S.phi[h.vi] = polar (1.0, - ruf.get() * 2.0 * PI);
    }
#ifdef COMPILE_PLOTTING
    ...
    normalpsi = L.getArgPrincipal(S.phi);
