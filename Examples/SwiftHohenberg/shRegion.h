...
public:
    ...
    vector<vector<FLT>> sortedBoundaryA; // for each region A values sorted by angle
    ...
    std::map<int,vector<FLT>> edgeA; //map of (i,j) edgeA values, uses pair<->int converter
    ...
    vector<vector<FLT>> A, B; //hold the field values for each hex in a region
    vector<FLT> a, b; //hold the field values for each hex in the whole hexGrid
  ...
  */
   ...
   //these are the vectors of vectors for the regions
        ...
        sortedBoundaryA.resize(NUMPOINTS);//hexes on region boundary sorted by angle
       ...
       for (unsigned int j=0;j<NUMPOINTS;j++){
           for (auto h : Hgrid->hexen) {
               if (region[h.vi][0] == (int) j) {
                   //ccout << "hex " << h.vi << " region " << region[h.vi][0] << " j " << j<< endl;
  ...
  //function to return mean value of A in the region
  FLT meanA (int regNum) {
    FLT mean = 0;
    int size = (int) this->regionHex[regNum].size();
    cout << "in meanA size " << size << endl;
    for (int i=0; i<size; i++){
        cout << " in loop i " << i << " mean " << mean << " regNum " << regNum <<  endl;
        mean += this->A[regNum][i];
    }
    if (size != 0) {
...
//function to print a vector
  ...
  //function to return fraction of area with A positive
  FLT regAfrac (int regNum) {
    FLT area = 0;
    FLT positive_area = 0;
    //int size = this->regionHex[regNum].size();
    vector<FLT> normalA;
    // to normalise the A field
     for (auto h : this->regionHex[regNum]){
          normalA.push_back(this->A[regNum][h.vi]);
      }
      normalA = this->meanzero_vector(normalA);
...
// function to give r and theta relative to region centre
    pair <FLT,FLT> set_polars(int regNum){
        ...
        FLT minPhi = 10.0;
        FLT maxPhi = -10.0;
        cout << "in set_polars region " << regNum << " size " << this->regionHex[regNum].size() << endl;
...
// tempvect1 and 2 have the values of A on the edge with the mean of the region subtracted
...
// tempvect1 and 2 have the values of A on the edge with the mean of the region subtracted
    ...
    void random_correlate(const int max_comp, const int morphNum,  bool lzero=true) {
        ofstream jfile;
        ofstream kfile;
        ...
        while (count <max_comp) {
           ...
           //FLT Amean1 = this->meanA(reg1);
           FLT Amean1=0;
           cout << "after meanA call" << endl;
           //FLT Amean2 = this->meanA(reg2);
           FLT Amean2=0;
            if ((s1 != 0) && (s2 != 0)) { //neither edge is empty
                ...
                for (auto itr = this->edges[rr1].begin(); itr != this->edges[rr1].end();itr++) {
                    first.push_back(this->A[reg1][*itr]);
                }
                first = this->meanzero_vector(first, Amean1);
                for (auto itr = this->edges[rr2].begin(); itr != this->edges[rr2].end();itr++) {
                    second.push_back(this->A[reg2][*itr]);
                }
                second = this->meanzero_vector(second,Amean2);
               if (s1 < s2) {
    ...
    void Srandom_correlate(const int max_comp, const int morphNum, bool lzero=true) {
        ofstream jfile;
        ofstream kfile;
        int max_rand = edgeIndex.size();
        string str = to_string(morphNum);
           ...
           int edge2Size =edges[rr2].size();
            if ((s1 != 0) && (s2 != 0)) { //neither edge is empty
                ...
                for (int i=0; i<edge1Size; i++) {
                    first.push_back(this->A[reg1][i]);
                }
                first = this->meanzero_vector(first, Amean1);
                for (int i=0; i<edge2Size; i++) {
                    second.push_back(this->A[reg2][i]);
                }
                second = this->meanzero_vector(second,Amean2);
                if (s1 < s2) {
                    dinterp = equalize_vector(first , second);
...
// use of Creg makes it only relevant to unmorphed code
    ...
    vector<FLT> ARegional(int regNum) {
        vector<FLT> result;
        for (auto h : regionHex[regNum]) {
            result.push_back(A[regNum][h.vi]);
        }
        return result;
    }
     ...
     * 2. A values of the hexes in angular order around the boundary
    ...
    void sortRegionBoundary(int regNum) {
        ...
        for (int i=0; i< bsize; i++) {
            ...
            this->sortedBoundaryA[regNum].push_back(A[regNum][regionBoundary[irB[i]]]);
        }
    }

    /* sectorize over radius analogue version
     * unlike the old region.h we cannot directly fill from the A field because this is different when we integrate
 ...
 // to normalise the A field
        ...
        for (unsigned int i=0; i<fieldVal.size(); i++){
            normalA.push_back(fabs(fieldVal[i]));
        }
        dfile << "after normalise field "<< field << endl;
//for (int i=0;i<size;i++)
// dfile << " i " << i << " normalA[i] " << normalA[i] << endl;
        ...
        if (radiusCount[k] == 0)
            ...
            radiusA[k] = radiusA[k]/radiusCount[k];
        dfile << " region " << regNum << " startradius "<<startradius<<"  finishradius "<<finishradius<< " radiusA " << radiusA[k] << endl;
...
// to normalise the A field
       normalA = meanzero_vector(normalA);
       FLT epsilon = 0.0001*(this->maxVal(normalA) - this->minVal(normalA));
 //for (int i=0;i<size;i++)
 // dfile << " i " << i << " normalA[i] " << normalA[i] << endl;
      for (int k=0;k<numSectors;k++) {
         startradius = (k*radiusInc);
...
//radiuscc[k] += this->cc[this->regionHex[regNum][i]];
      ...
      for (int k=0;k<numSectors;k++){
         ...
         dfile << " region " << regNum <<" startradius "<<startradius<<"  finishradius "<<finishradius<< " DradiusA " << radiusA[k] << endl;
    ...
    //std::pair<FLT,FLT> diff; //difference between seed point and CoG of region
        ...
        vector <FLT> angleA; //average value of cc in each sector
        vector <FLT> normalA;
        vector <int> angleCount; //number of hexes in each sector
        angleA.resize(numSectors,0);
...
//int size = (int) this->regionHex[regNum].size();
        for (unsigned int i=0; i<fieldVal.size(); i++){
            normalA.push_back(fieldVal[i]);
        }
        /*
        for (auto h : this->regionHex[regNum]){
            normalA.push_back(fieldVal[h.vi]);
        }
        */
        normalA = meanzero_vector(normalA);
//for (int i=0;i<size;i++)
//   cfile << " i " << i << " normalA[i] " << normalA[i] << endl;
...
//cfile << setw(5) << angleVal[i]  <<"  ";
        ...
        angleA = meanzero_vector(angleA);
        for (int k=0;k<numSectors;k++){ //calculate the average angle in the sector
            startAngle = k*angleInc;
            endAngle = k*angleInc;
            if (angleCount[k] != 0)
                angleA[k] = angleA[k]/(1.*angleCount[k]);
            else
                angleA[k] = -999.999;
//write out values
            cfile << " region " << regNum <<" startAngle "<< startAngle << "  endAngle "<< endAngle << " angleA " << angleA[k] << endl;
        }//end loop on sectors
    ...
    vector <int> sectorize_reg_Dangle (int regNum, int numSectors, int beginradius, int endradius, vector<FLT> fieldVal) {
        ofstream cfile ("logs/sectorAngle.txt",ios::app);
 //std::pair<FLT,FLT> diff; //difference between seed point and CoG of region
        vector <int> angleA; //digitized value of A in each sector
...
//angleHold[k] = angleHold[k] / (1.*angleCount[k]);
            ...
            cfile << " region " << regNum <<" startangle  " << startAngle << "  endAngle  "<< endAngle << " DangleA " << angleA[k] << endl;
        } //end loop over sectors
        return angleA;
  ...
  //method to renew polars and boundary
    ...
    int regionBessel(int regNum, int radialOrder, int angularOrder, FLT phase, FLT Dn=1.0) {
        ...
        for (auto h : this->regionHex[regNum]) {
            ...
            A[regNum].push_back(boost::math::cyl_bessel_j(radialOrder, radius) * cos(angularOrder * angle));
 ...
 */
    void renewDissect(int regNum, int morphNum) {
        cout << "in renewDissect region " << regNum << endl;
        string str = to_string(morphNum);
       ...
       for (int i = 0; i < bsize; i++)
       {
        ...
        cout << "end of  renewDissect region " << regNum << endl;
    ...
    FLT correlate_edges(int iter, bool lzeroMean=true)
    {
        ...
        for (unsigned int i = 0; i <NUMPOINTS; i++)
        {
            ...
            FLT Amean1, Amean2;
            Amean1 = this->meanA(i); //find the mean of nn in the region
            edgefile << " mean of nn in region " << i << "is " << Amean1 << endl;
            for (auto j = this->regionList[i].begin(); j < this->regionList[i].end();j++)
            {
                if (*j == -1) continue;
                Amean2 = this->meanA(*j); //find the mean of nn in the region
                edgefile << " j iteration " << *j << " Amean2 " << Amean2 << endl;
                first.resize(0);
                second.resize(0);
                dinterp.resize(0);
                std::pair<int,int> edgePair1(i,*j);
        ...
        correl.close();
        return result;
    } //end of function correlate_edge
    ...
    FLT renewcorrelate_edges(int regNum,  const int morphNum, bool lZero=true)
    {
        ...
        ofstream edgefile(this->logpath + "/edgeCorrelations" + str + ".txt",ios::app);
        edgefile << " in morphed edge correlation routine lZero " << lZero << endl;
        ...
        int countResult = 0;
        FLT Amean1, Amean2;
        Amean1 = this->meanA(regNum);
        edgefile << " mean of A in region " << regNum << "is " << Amean1 << endl;
        for (auto j = this->regionList[regNum].begin(); j < this->regionList[regNum].end(); j++)  {
            if (*j == -1) continue;
            Amean2 = this->meanA(*j); //find the mean of A in the region
            edgefile << " j iteration " << *j << " region " << regNum <<" Amean2 " << Amean2 << endl;
            first.resize(0);
            second.resize(0);
            dinterp.resize(0);
            ...
            for (auto itr = this->edges[edgeIndex1].begin(); itr != this->edges[edgeIndex1].end();itr++)
            {
                first.push_back(this->A[regNum][*itr]);
                count1++;
            }
            first = this->meanzero_vector(first, Amean1);
            for (auto itr = this->edges[edgeIndex2].begin(); itr != this->edges[edgeIndex2].end();itr++)
    ...
    FLT Srenewcorrelate_edges(int regNum,  const int morphNum, bool lZero=true)
    {
        ...
        for (auto j = this->regionList[regNum].begin(); j < this->regionList[regNum].end(); j++)  {
            ...
            for (int i=0; i<edge1Size; i++)
            {
                if (isnan(this->A[regNum][i]))
                     edgefile << "Nan  edge hex 1 " << i <<  endl;
                first.push_back(this->A[regNum][i]);
                count1++;
            }
            first = this->meanzero_vector(first, Amean1);
