...
//section for rectangular grid
    ...
    int ny = S.n / nx;
    int nrows = S.Hgrid->d_rowlen;
    int ncols = S.Hgrid->d_numrows;
    std::cout << "nx " << nx << " ny " << ny << " rowlen " << S.Hgrid->d_rowlen << " numrows " << S.Hgrid->d_numrows <<  std::endl;
    std::cout << "width " << S.Hgrid->width() << std::endl <<  " depth " << S.Hgrid->depth() << std::endl;
    if (nx*ny != S.n) {
    ...
    */
    ...
    for (int i=0;i<numsteps;i++) {
        ...
        if (i % numprint == 0) {
            ...
            cv::Mat I = L.vect2mat(psiphase, nrows, ncols);
