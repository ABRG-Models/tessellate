...
// method to determine if a point is in a polygon
   ...
   // return the mean of the values of a  vector
    ...
    void setR(vector<FLT> invector>) {
        if (static_cast<int>invector.size() != this->n) {
            cerr << " setR: invector " << invector.size() << " not equal to this->n " << this->n << endl;
            return;
        }
        ...
        if (count != invector.size()) {
            cerr << " setR: number of hexes " << this.n << " not equal assigned h.r " << count << endl;
        }
    } //end of setR


    void setPhi(vector<FLT> invector>) {
        if (static_cast<int>invector.size() != this->n) {
            cerr << " setPhi: invector " << invector.size() << " not equal to this->n " << this->n << endl;
            return;
        }
        unsigned int count = 0;
        for (auto& h : this->hexen) {
