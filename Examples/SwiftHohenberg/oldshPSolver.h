...
// constructor with radius passed in for solving on radial boundaries
    shSolver (int scale, FLT xspan, string logpath, float radius, pair<float, float> seedPoint, FLT lengthScale, FLT axisRatio=1.0f, bool lCircle=true) {
        ...
        if (lCircle) {
            ...
            FLT longRadius = radius*axisRatio;
            Hgrid->setEllipticalBoundary(radius, longRadius, seedPoint, false);
        }
...
//counts the pinhwheels in an RoI
    int pinCountInRoI(vector<bool> A,  FLT halfWidth, std::pair<FLT,FLT> centre) {
        int result = 0;
        std::pair<FLT,FLT> botLeft; //coordinates of bottom left
        std::pair<FLT,FLT> topRight; //coordinates of top right
        botLeft.first = centre.first - halfWidth;
        botLeft.second = centre.second - halfWidth;
        topRight.first = centre.first + halfWidth;
        topRight.second = centre.second + halfWidth;

        for (int di=0; di<this->n; di++) {
            if (A[di]) {
                FLT X = this->Hgrid->d_x[di];
                FLT Y = this->Hgrid->d_y[di];
                if (X > botLeft.first && X < topRight.first && Y > botLeft.second && Y < topRight.second) {
                    //std::cout << " pinwheel at " << X << " , " << Y << std::endl;
                    result++;
                }
            }
        }
...
//finds the coordinates of the pinWheel centres returns them in a vector
    std::vector<morph::Vector<FLT,3>> pinWheelCoords(std::vector<bool> A) {
        std::vector<morph::Vector<FLT,3>> result;
        int count = 0;
        for (int di=0; di<this->n; di++) {
            if (A[di]) {
                FLT X = this->Hgrid->d_x[di];
        ...
        std::cout << " number of pinWheels " << count << std::endl;
        return result;
    }
