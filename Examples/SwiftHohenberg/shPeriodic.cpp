...
// kludge need to include in json file
...
//  set up random initial conditions
    for (int di=0; di<S.n; di++) {
        FLT choice = ruf.get();
        S.psi[di] = std::polar (1.0f, - choice * 2.0f * 3.1415927f);
        FLT choice1 = ruf.get();
        choice1 = ruf.get();
        S.phi[di] = std::polar (1.0f, - choice1 * 2.0f * 3.1415927f);
    }
//
/* set up sinusoidal initial conditions
    int di = 0;
    for (int i = 0; i<S.Hgrid->d_numrows; i++) {
        FLT eta = i * 12.0 * PI/(S.Hgrid->d_numrows);
        for (int j=0; j<S.Hgrid->d_rowlen; j++) {
            FLT theta = j * 12.0 * PI/(S.Hgrid->d_rowlen * 1.0);
            di = i*S.Hgrid->d_rowlen + j;
            S.psi[di] = std::polar (1.0f, - sin(eta) * sin(theta));
...
//    morph::HexGridVisual<FLT>* hgv2 = new morph::HexGridVisual<FLT> (v1.shaderprog, v1.tshaderprog, S.Hgrid, spatOff);
    ...
    for (int i=0;i<numsteps;i++) {
	    ...
	    normalpsi.resize(S.n);
            normalpsi = L.getArgPrincipal(S.psi);
            cerr << "max arg of normalpsi  " << L.maxVal(normalpsi) << " min arg of normalpsi " << L.minVal(normalpsi) <<  " iteration " << i <<endl;
