...
// Constructor for parallelogram domain
    ...
    FLT hexWidth = S.Hgrid->width();
    FLT hexDepth = S.Hgrid->depth();
    FLT widthAdjust =  hexDepth/1.7321;
...
//Now set i.c.s
    ...
    */
    ...
    // A. move a hexDepth above.
    xzero = 0.0 + widthAdjust;
    yzero = hexDepth + 0.025;
    spatOff = { xzero, yzero, 0.0 };
    ...
    hgv3->addLabel ("Psi modulus", {-0.05f, txtoff, 0.0f}, morph::colour::black, morph::VisualFont::VeraSerif, 0.05, 56);
    //hgv1->addLabel ("Variable A", { -0.2f, RD.ellipse_b*-1.4f, 0.01f },
    //                morph::colour::white, morph::VisualFont::Vera, 0.1f, 48);
    // "finalize" is required before adding the HexGridVisual to the morph::Visual.
    hgv3->finalize();
    ...
    // move left a dexWidth.
    xzero = -hexWidth - 0.025;
    yzero = 0.0;
    spatOff = { xzero, yzero, 0.0 };
    morph::HexGridVisual<FLT>* hgv4 = new morph::HexGridVisual<FLT> (v1.shaderprog, v1.tshaderprog, S.Hgrid, spatOff);
    hgv4->setScalarData (&psiphase);
    // Z position scaling - how hilly/bumpy the visual will be.
    hgv4->zScale.setParams (0.0f, 0.0f);
    // The second is the colour scaling. Set this to autoscale.
    hgv4->colourScale.do_autoscale = true;
    hgv4->cm.setType (cmt);
    hgv4->hexVisMode = morph::HexVisMode::HexInterp;
    hgv4->addLabel ("Phi modulus", {-0.05f, txtoff, 0.0f}, morph::colour::black, morph::VisualFont::VeraSerif, 0.05, 56);
    hgv4->finalize();
    v1.addVisualModel (hgv4);

    // A. move a hexGrid right.
    // move back a whole hexGrid width
    xzero = hexWidth + 0.025;
    yzero = 0.0;
    spatOff = { xzero, yzero, 0.0 };
    morph::HexGridVisual<FLT>* hgv5 = new morph::HexGridVisual<FLT> (v1.shaderprog, v1.tshaderprog, S.Hgrid, spatOff);
    hgv5->setScalarData (&phiphase);
    // Z position scaling - how hilly/bumpy the visual will be.
    hgv5->zScale.setParams (0.0f, 0.0f);
    // The second is the colour scaling. Set this to autoscale.
    hgv5->colourScale.do_autoscale = true;
    hgv5->cm.setType (cmt);
    hgv5->hexVisMode = morph::HexVisMode::HexInterp;
    hgv5->addLabel ("Phi phase", {-0.05f, txtoff, 0.0f}, morph::colour::black, morph::VisualFont::VeraSerif, 0.05, 56);
    hgv5->finalize();
    v1.addVisualModel (hgv5);
#endif
    ...
    */
    ...
    for (int i=0;i<numsteps;i++) {
        ...
        if (i % numprint == 0) {
            ...
            hgv2->updateData (&psiphase);
            hgv2->clearAutoscaleColour();

            hgv3->updateData (&psiphase);
            hgv3->clearAutoscaleColour();

            hgv4->updateData (&psiphase);
            hgv4->clearAutoscaleColour();

            hgv5->updateData (&psiphase);
            hgv5->clearAutoscaleColour();
            if (saveplots) {
                if (vidframes) {
