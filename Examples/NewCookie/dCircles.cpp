...
int main (int argc, char **argv)
{
    ...
    double Dchi = conf.getDouble("Dchi",36.0);
    double Dc = conf.getDouble("Dc",10.8);
    ...
    double ccInitialOffset = conf.getDouble("ccInitialOffset", 0.0);
    ...
    std::cout << "Dn " << Dn << " Dchi " << Dchi << " Dc " << Dc << " diffTol " << diffTol << " numpoints " << numpoints << " Lperturb " << Lperturb << " LDn " << LDn << std::endl;
    std::cerr << "Dn " << Dn << " Dchi " << Dchi << " Dc " << Dc << " diffTol " << diffTol << " numpoints " << numpoints << " Lperturb " << Lperturb << " LDn " << LDn << std::endl;
    ...
    ofstream afile (logpath + "./schDebug.out");
...
// include the analysis methods
    ...
    for (int j=0; j<numpoints;j++) {
        S[j].setParams(Dn, Dchi, Dc);
    }
    cout << "first setting of centroids" << endl;
    for (unsigned int j=0; j<numpoints; j++){
        cout << "centroid region " << j << " is ( " << M.centroids[j].first << " , " << M.centroids[j].second << " )" << endl;
    }
    ...
    for (unsigned int j=0; j<numpoints;j++){
        cout << " Region size " << M.regionHex[j].size() << endl;
    }
...
// initialise with random field
        ...
        for (unsigned int j=0;j<numpoints;j++)
	{
            ...
            if (choice > 0.5)			{
                ...
                cout << "j " << j <<" choice " << choice << " h.vi " << h.vi << " S[j].A[h.vi]  " << S[j].A[h.vi] <<endl;
...
//first save the  ofstream outFile;
	...
	//	std::copy(ccstr.begin(),ccstr.end(),cst);
        std::strcpy(ccst,ccstr.c_str());
        cout << "labels "<< nst <<" , " << nstr <<","<< ccst<< "," << ccstr <<endl;
        data.add_contained_vals(ccst,S[j].B);
        data.add_contained_vals(nst,S[j].A);
    }
...
// look at correlation between adjacent edges
    ...
    //FLT avAbsCorrelation = M.correlate_edges(0);
...
// section for solving on the circle Tessllation
    ...
    for (int j=0; j<numpoints; j++) {
        ...
        S.push_back(schSolver(scale, xspan, logpath, radius, perturbCentres[j], lengthScale));
        cout << "in the loop populating the schVector"<< j << " centroid.x " << perturbCentres[j].first - M.baryCentre(j).first << " centroid.y " << perturbCentres[j].second - M.baryCentre(j).second << endl;

    } //end of loop over NUMPOINTs setting Dn etc..
...
// initialise with random field
        ...
        for (unsigned int j=0;j<numpoints;j++) {
            ...
            // normal value. Close to boundary, noise is less.
                ...
                if (choice > 0.5)   {
                    ...
                    S[j].B[h.vi] = - ruf1.get() * aNoiseGain + ccInitialOffset;
                }
		else	{
                    S[j].A[h.vi] =  ruf1.get() * aNoiseGain +nnInitialOffset;
                    S[j].B[h.vi] =  ruf1.get() * aNoiseGain + ccInitialOffset;
    ...
    // begin time stepping loop cutter grid solved via schSolver
    // set up vectors for determining convergence
    ...
    //now set the parameters for each solver
    for (int j=0; j<numpoints;j++) {
        S[j].setParams(DnVal[j], DchiVal[j], DcVal[j]);
    }
